...
public class DashboardViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenSreamProcessed_then_UPDATE_1(@Payload SreamProcessed sreamProcessed) {
        try {
            if (!sreamProcessed.validate()) return;
                // view 객체 조회

                List<Dashboard> dashboardList = dashboardRepository.findByFileId(sreamProcessed.getFileId());
